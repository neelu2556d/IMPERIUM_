...
A forkable engine for building **Imperium-style dashboard tiles**. It's not the app and
...
| `dna/` | The full Imperium design DNA pack: theme, motion, voice, icons, components, the `gotchas` rulebook, per-feature recipes, and complete worked-example tiles. |
...
tracks cold plunges, in the Imperium style."* Claude reads `ENGINE.md` + `dna/` +
...
```
See engine/ENGINE.md for how to build Imperium-style tiles (design DNA + real tiles + self-check).
```

**B. The live connector (no fork).** The same engine is served by the Imperium MCP as
resources — connect it once and the context just loads, always the newest DNA:

- `imperium://engine` — the orientation brain
- `imperium://dna/{name}` — any DNA section (`theme`, `motion`, `voice`, `gotchas`, `feature-vitals`, …)
