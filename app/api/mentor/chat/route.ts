...
import { ASK_GLYPH_NAMES, resolveAskGlyph } from '@/lib/I/askGlyphs'
...
const SYSTEM_PROMPT_PREAMBLE = `You are I, the Imperium mentor. One mind that has read every part of this user's life in the app: training, fuel, vitals, goals, money, how they feel (their mood), what is going on in their life, and the thoughts they throw into the void. You are always in their corner.
...
/*
 * Shared-brain write path. I appends durable facts after a marker the UI
 ...
 * useful reply, I appends a hidden ===ASK=== block (parallel to ===MEMORY===)
...
/** Split I's raw output into the visible reply + an optional ask card + facts.
...
export async function POST(request: NextRequest) {
  ...
  const [
    ...
    // the user's OWN built tiles (report contract) — so I can talk about the
  ...
  // The same fused daily read the Vitals page shows, so I references today's
  ...
  // Their OWN tiles (Phase 2 of I-as-the-middle). One dense line per stream,
  // capped at 8, built by the tested formatter — the user BUILT these trackers,
  // so I referencing them by name is the "it knows my whole board" moment.
  ...
  // "I connects" — deterministic cross-stream links (tile x recovery/sleep/
  // training), verified by the gated scan, so I can CITE a real pattern
  ...
  // I makes (feelings x sleep/training/spending). Sits next to the wearable data
  ...
  // Today's fused signal — the same read the Vitals page shows. Keeps I
  ...
  // The shared brain. Every module writes user_facts; I reads the most
  ...
  // Life context the user told I directly (the "Folded Notes"). Backstory I
  ...
  try {
    ...
    if (typeof text !== 'string') {
      return NextResponse.json({ error: 'Empty response from I' }, { status: 502 })
    }
