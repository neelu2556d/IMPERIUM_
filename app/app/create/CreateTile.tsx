 ...
 *
 *   tile  to host : { source:'Imperium-tile', type:'save', data }
 *   tile  to host : { source:'Imperium-tile', type:'load', id }
 *   tile  to host : { source:'Imperium-tile', type:'report', stream }
 *   host  to tile : { source:'Imperium-host', type:'load:result', id, data }
 *
 * save/load are the tile's OWN data. report is the one numeric life-stream into
 * I (the noticed brain): the host forwards it to the reportStream server
 * action, which validates it and RLS-writes it under the session user. This is
 * the quiet magic, a tile auto-connects to I just by reporting.
 *
...
var Imperium={_w:{},_sv:false,save:function(d){Imperium._sv=true;parent.postMessage({source:'Imperium-tile',type:'save',data:d},'*')},load:function(){return new Promise(function(res){var id=Math.random().toString(36).slice(2);var ask=function(){parent.postMessage({source:'Imperium-tile',type:'load',id:id},'*')};var n=0;var t=setInterval(function(){n++;if(n===1){ask();return}clearInterval(t);if(Imperium._w[id]){Imperium._w[id]=function(d){if(d!=null&&!Imperium._sv)location.reload()};res(null)}},3000);Imperium._w[id]=function(d){clearInterval(t);res(d)};ask()})},report:function(s){parent.postMessage({source:'Imperium-tile',type:'report',stream:s},'*')}};
 ...
 * including Imperium.report(), so a tile built outside auto-connects to I:
...
2. Imperium.report(stream) feeds I, the dashboard's brain. Report the tile's ONE main number every time the user logs it, right next to the save:
...
3. One stream per tile. Reporting is what connects the tile to my daily score and lets I notice patterns across my life.
...
function asEnvelope(raw: string): TileEnvelope | null {
  const text = raw.trim()
  if (!text) return null
  // A friend's shared tile arrives as a `Imperium:tile:` code; unpack it first.
  ...
  const { register, unregister } = useTileHost(
    ...
    ({ type, count }) => {
      ...
      if (type === 'report') {
        setSavedOn(true)
        setSaving(false)
        setStatusLabel('Reported to I. The noticed brain is watching this stream.')
      } else if (type === 'save') {
        setSavedOn(true)
        setSaving(false)
    ...
    // the "Reported to I" activity when the write actually landed; a failure
          ...
          if (!res.ok) {
            ...
            setStatusLabel(
              res.error === 'unauthorized'
                ...
                : 'That report did not land, so I has not seen it. Log it again in a moment.',
            )
          }
          return res
        })
        // A transport-level rejection (network drop mid-flight) must flip the
        // status line honest too, not leave an earlier "Reported to I" lying.
        .catch(() => {
          setSaving(false)
          setSavedOn(false)
          setStatusLabel('That report did not land, so I has not seen it. Log it again in a moment.')
  ...
  function copyPrompt() {
    ...
    navigator.clipboard
      ...
      .then(() =>
        setRunMsg({
          kind: 'ok',
          text: 'Build prompt copied. Paste it into Claude Code in VS Code, or claude.ai, and it will build you a tile that reports to I.',
        }),
      )
      .catch(() =>
