 ...
 * the user's recent data and phrases it the way I would — warm, specific, no
...
export interface VentTeaser {
  ...
  /** I's reply */
  I: string
  /** substring of `I` to emphasise (bold), if any */
  emphasis?: string
...
export const SAMPLE_TEASER: VentTeaser = {
  user: USER_LINE,
  I: 'Once you have logged a few days, I will notice things like a run of short nights or a heavy training week, and gently point to the one that is really weighing on you. Nothing is ever wrong with you.',
  real: false,
}
...
export function buildVentTeaser(s: VentSignals): VentTeaser {
  ...
  if (s.sleepNightsTracked >= 3 && s.sleepNightsUnder6 >= 2) {
    ...
    return {
      user: USER_LINE,
      I: `Look at your sleep this past week. You have been ${emphasis}. That alone makes everything feel heavier than it is. Nothing is wrong with you. Want to talk about what is keeping you up?`,
      emphasis,
      real: true,
    }
  }
  ...
  if (s.latestRecovery != null && s.latestRecovery < 40) {
    ...
    return {
      user: USER_LINE,
      I: `Your ${emphasis} today. Your body is asking for a gentler day, and that is completely okay. None of this is you failing. Want to talk it through?`,
      emphasis,
      real: true,
    }
  }
  ...
  if (s.sessionsDone7d >= 5) {
    ...
    return {
      user: USER_LINE,
      I: `You have ${emphasis}. That is a serious load, and feeling worn down is your body keeping score, not you slipping. Rest is part of the work. Want to talk about it?`,
      emphasis,
      real: true,
    }
  }
