...
export default function ContextNotes({ initialFacts }: { initialFacts: ContextFact[] }) {
  ...
  if (closed) {
    return (
      <section className={styles.contextSec} aria-label="what I should know">
        <div className={styles.secHead}>
          <span className={styles.secEyebrow}>context</span>
          <span className={styles.secTitle}>What I should know</span>
          <span className={styles.secRule} aria-hidden />
        </div>
        <p className={styles.ctxClosed}>That is plenty for now. I can take it from here.</p>
      </section>
    )
  }
  ...
  return (
    <section className={styles.contextSec} aria-label="what I should know">
      ...
      {showFree ? (
        ...
        <button className={styles.freeLink} type="button" onClick={() => setShowFree(true)}>or tell I something in your own words</button>
      )}
    </section>
  )
}
