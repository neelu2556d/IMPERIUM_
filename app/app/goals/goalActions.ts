...
/**
 * I Goals — server actions (Phase 1 CRUD).
 *
...
/**
 * Keep I's shared memory in lockstep with a goal's current state. The policy
...
export async function createBigGoal(input: CreateBigGoalInput): Promise<Ok<{ goal: BigGoal }> | Err> {
  ...
  // Feed I's shared memory so the goal surfaces in chat (the chat route
...
export async function updateBigGoalProgress(id: string, current: number): Promise<Ok<{ goal: BigGoal }> | Err> {
  ...
  const goal = rowToBigGoal(data as BigGoalRow)
  // Progress moved — refresh the memory so I knows where they are now.
  await syncGoalFact(supabase, user.id, goal)
...
/** Change how much I shows up about a goal, and sync its memory to match —
 ...
 * goal, or clear it (null = let I decide). buildTicker honors the override
...
export async function setBigGoalStatus(id: string, status: BigGoalStatus): Promise<Ok<{ goal: BigGoal }> | Err> {
  ...
  // Keep I's memory current: achieved -> a celebration fact, paused/abandoned
...
export async function deleteBigGoal(id: string): Promise<Ok<unknown> | Err> {
  ...
  // The goal is gone — remove anything I remembered about it, so it never
...
export async function categorizeAndCleanGoal(goalId: string): Promise<Ok<{ goal: BigGoal }> | Err> {
  ...
  const goal = rowToBigGoal(data as BigGoalRow)
  // Refresh I's memory so it uses the tidy title once we have one.
  await syncGoalFact(supabase, user.id, goal)
