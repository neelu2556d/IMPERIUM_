/**
 * Tile -> I bridge logic: turns a reported stream (the report contract) into
 ...
 * actually handed I a categorized life-stream" true, with zero per-tile code.
 *
 ...
 *   - and, with goalDirection, the sign I should expect (a "down" habit like
...
/** The sign I should expect between a stream and a higher-is-better outcome
 ...
 * gated insight ONLY when the link is real, or null (I stays quiet). `outcome`
...
/* --------------------------------------------------------------------------
 * Per-tile "what I noticed": the single deterministic line a tile shows on
...
export function tileInsight(history: TileHistory): TileInsight {
  ...
  if (n === 1) {
    return { text: 'One day in. Log tomorrow and I starts spotting your pattern.', tone: 'good', kind: 'starting', stat: 1 }
  }
  if (n === 2) {
  ...
  return { text: `${n} days logged and holding. Keep it going and I will call the trend.`, tone: 'neutral', kind: 'quiet', stat: n }
}
