/**
 * Tile context — the compact "their tiles" block for I's chat brain. Turns a
 * user's reported streams into a handful of dense, factual lines the mentor
 * route can drop into its context block, so I can talk about the tiles the
...
function lineFor(d: StreamDigest, keys7: string[], keys28: string[]): string {
  ...
  // alcohol), plus the stated goal so I reads the number the right way.
  const parts: string[] = []
