 ...
 * nudge" (I stays quiet rather than crashing the I tab).
 */
...
export async function gatherDrift(
  ...
  try {
    ...
    const [wRes, mRes, recRes, ndRes, gRes, tsRes, trRes] = await Promise.all([
      ...
      // active, non-silent goals — so I can reach out about the SPECIFIC goal
    ...
    for (const row of gRes.data ?? []) {
      ...
      // raw, possibly-lowercase user sentence into I's outreach.
