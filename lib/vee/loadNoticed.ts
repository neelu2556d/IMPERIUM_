/**
 * loadNoticed - the server-side loader for the new I surface (/app/mentor-next).
...
export async function loadNoticed(supabase: Db, userId: string): Promise<NoticedData> {
  try {
    ...
    // override-bound goals from them, and "I connects" scans them below.
    ...
    // "I connects": the user's own tile streams scanned against the core outcomes.
    ...
    // The rarity collection + run stats: honest proof I has been on it.
