...
/** The user's top goal as set in the goals tab / I (lib/goals BigGoal). The
 *  coach grades by the SAME goal, so "lose weight" or "get leaner and stronger"
 *  in I drives the Fuel grade. Loose shape so Fuel never imports I's types. */
...
/** Read a mode from the I goal's words. Build is checked first so "leaner AND
 ...
 * Precedence: an explicit nutrition outcome wins; otherwise the user's top I
 ...
 * `bigGoal` is the user's top active goal from I (so the two stay in sync).
 */
export function deriveGoalMode(input: {
  ...
  // Otherwise honor the goal the user set in I (so the coach and I agree).
