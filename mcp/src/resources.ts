// MCP resources for the Imperium Tile Engine — the CONNECTOR half of the engine.
//
...
// RESOURCES — so once the Imperium MCP is connected, "build me a tile that looks
...
const CANON_DOMAINS = [
  ...
  'water', 'peak', 'finance', 'brand', 'mentor', 'I',
];
...
export function registerResources(server: McpServer): void {
  ...
  server.registerResource(
    ...
    'Imperium://engine',
    {
      title: 'Imperium Tile Engine — start here',
      description:
        'How to build a Imperium-native dashboard tile: the bar, the sealed-tile bridge contract, and the reference index. Read this first, then pull a domain kit or a DNA section.',
      mimeType: MD,
    },
    async (uri) => ({
  ...
  server.registerResource(
    'Imperium-dna',
    new ResourceTemplate('Imperium://dna/{name}', {
      list: async () => ({
        resources: listSections().map((s) => ({
          uri: `Imperium://dna/${s.name}`,
          name: s.name,
          title: `Imperium DNA — ${s.name}`,
          description: `Reference section (${s.folder}) from the Imperium design DNA pack.`,
          mimeType: MD,
        })),
      }),
    }),
    {
      title: 'Imperium DNA section',
      description:
        'One reference file from the Imperium design DNA pack: theme, motion, voice, icons, components, the gotchas rulebook, per-feature recipes, and complete worked-example tiles.',
      mimeType: MD,
    },
    async (uri, variables) => {
  ...
  server.registerResource(
    'Imperium-kit',
    new ResourceTemplate('Imperium://kit/{domain}', {
      list: async () => ({
        resources: CANON_DOMAINS.map((d) => ({
          uri: `Imperium://kit/${d}`,
          name: d,
          title: `Imperium tile kit — ${d}`,
    ...
    {
      title: 'Imperium tile kit (by domain)',
      description:
