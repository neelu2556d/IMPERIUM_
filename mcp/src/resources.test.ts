...
test('registerResources: registers the three engine resources', () => {
  ...
  assert.ok(res['Imperium-engine'], 'orientation resource');
  assert.ok(res['Imperium-dna'], 'dna section template');
  assert.ok(res['Imperium-kit'], 'domain kit template');
  assert.equal(res['Imperium-engine'].config.mimeType, 'text/markdown');
});

test('Imperium://engine returns the orientation brain', async () => {
  const { server, res } = fakeServer();
  registerResources(server);
  const out = await res['Imperium-engine'].read(new URL('Imperium://engine'));
  assert.match(out.contents[0].text, /Imperium TILE KIT/); // buildKit orientation header
  assert.equal(out.contents[0].mimeType, 'text/markdown');
});

test('Imperium://dna/{name} reads a real section and lists the whole pack', async () => {
  const { server, res } = fakeServer();
  registerResources(server);

  const out = await res['Imperium-dna'].read(new URL('Imperium://dna/theme'), { name: 'theme' });
  assert.ok(out.contents[0].text.length > 100, 'theme section has real content');

  const listed = await res['Imperium-dna'].uriOrTemplate.listCallback({});
  const names = listed.resources.map((r: { name: string }) => r.name).sort();
  assert.deepEqual(names, listSections().map((s) => s.name).sort());
  assert.ok(listed.resources.every((r: { uri: string }) => r.uri.startsWith('Imperium://dna/')));
});

test('Imperium://dna/{name} unknown name returns a helpful list, never throws', async () => {
  const { server, res } = fakeServer();
  registerResources(server);
  const out = await res['Imperium-dna'].read(new URL('Imperium://dna/nope'), { name: 'nope' });
  assert.match(out.contents[0].text, /No DNA section "nope"/);
});

test('Imperium://kit/{domain} bundles the BASE pack plus the domain reference', async () => {
  const { server, res } = fakeServer();
  registerResources(server);
  const out = await res['Imperium-kit'].read(new URL('Imperium://kit/vitals'), { domain: 'vitals' });
