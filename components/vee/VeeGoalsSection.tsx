...
/**
 * VeeGoalsSection - the FULL goals engine fused into the new I page, wearing
 * the I-notice-preview board signature:
 *
...
const PUSH_CHIP: Record<Push, string> = { silent: 'I · silent', gentle: 'I · gentle', balanced: 'I · nudges', push: 'I · pushes' }
const PUSH_SHORT: Record<Push, string> = { silent: 'Silent', gentle: 'Gentle', balanced: 'Nudges', push: 'Pushes' }
/** Bars per level: how loud I gets, 1 (silent) to 4 (pushes). */
...
/** The 1-4 bars meter: filled bars = how loud I gets about this goal. */
...
/* Honest, deterministic cold-start line per goal category: what to feed I to
...
const FEED_ME: Record<GoalCategory, string> = {
  ...
  career: 'jot a note in I when something moves and I will keep the thread.',
  craft: 'jot a note in I when you finish a piece and I will keep the thread.',
  people: 'write a note in I after time well spent and I will start counting the good days.',
  mind: 'write a note in I each day and I will start measuring your good days.',
  general: 'write a note in I each day and I will start measuring your good days.',
}
...
function sourceLabel(source: HabitGoal['source']): string {
  switch (source) {
    ...
    case 'mentor': return 'from a note to I'
...
function claudeHref(title: string): string {
  const q = `My Imperium goal: "${title}". I is steering it with my real data. Help me plan this week to move it.`
...
export interface VeeGoalsSectionProps {
  ...
  activeModules?: GuideModule[]
  /** Design-harness mode (/I-live-preview): every action stays local, no server writes. */
  readOnly?: boolean
}
...
export default function VeeGoalsSection({ goals: initialGoals, habits: initialHabits, rows, guides, lifeChips, daysLogged, suggestionEvidence, tileStreams = [], activeModules = [], readOnly = false }: VeeGoalsSectionProps) {
  ...
  // "this week with Imperium" lives behind a quiet disclosure (v1.2), closed by
  ...
  // Change how much I shows up about a goal. Optimistic; revert if the write
  ...
  // "What steers this": persist the user's pick (or null = let I decide).
  ...
  const CreateBlock = (
    creating ? (
      <form ref={sheetRef} className={gs.createSheet} onSubmit={e => { e.preventDefault(); createGoal() }}>
        ...
        <div className={gs.field}>
          <span className={gs.fieldLabel}>How much should I push you?</span>
                    ...
                    <div className={ts.gdCard}>
                      ...
                      <div className={gs.bigGoalMeta}>
                        ...
                        <button
                          ...
                          aria-label={`change how much I shows up about ${title}`}
                      ...
                      {editPushId === g.id && (
                        <div className={gs.pushEdit} id={`push-edit-${g.id}`} role="group" aria-label="How much should I show up">
                          {PUSH_OPTS.map(o => (
                            <button
                              type="button"
                              key={o.key}
...
{/* The inline chip strip is retired (2026-07-12): "what steers
                          this" is THE GRAPH LIBRARY modal now - I's module
      ...
      {/* "this week with Imperium": the habit rows fold behind one quiet mono
      ...
      {(habits.length > 0 || openSuggestions.length > 0) && (
        <>
          ...
          >
            ...
            this week with Imperium · {habits.length + openSuggestions.length}
