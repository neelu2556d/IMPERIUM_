 ...
 * that exact line ('core:<id>' binding). I's module picks ride on top as
 * quick pills, "let I decide" hands the wheel back, and needs-new-data
 ...
 * any goal opens this - the I tab pays nothing until then.
 */
...
export default function GraphLibrary({
  ...
  /** I's module picks for this goal's category (label word + binding key). */
    ...
    <div className={s.overlay} role="dialog" aria-modal="true" aria-label={`Choose the graph for ${goalTitle}`}>
      ...
      <div className={s.card}>
        ...
        <div className={s.head}>
          ...
          {/* I's picks: the module lanes (analysed trends, tips, levers). */}
          <div className={s.quick} role="group" aria-label="I's picks">
            {quickPicks.map((p) => (
              <button
                type="button"
                key={p.key}
